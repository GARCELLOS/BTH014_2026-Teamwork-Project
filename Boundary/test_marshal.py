...
def generate_boundary_values():
    ...
    boundary_samples.append(
        ("float_max_minus", sys.float_info.max * 0.9999)
    )
    boundary_samples.append(
        ("float_min_plus", sys.float_info.min * 1.0001)
    )

    # --- complex ---
    boundary_samples.append(("complex_0", 0j))
    boundary_samples.append(
        ("complex_max", complex(sys.float_info.max, sys.float_info.max))
    )
    boundary_samples.append(
        ("complex_min", complex(sys.float_info.min, sys.float_info.min))
    )
    boundary_samples.append(
    ...
    boundary_samples.append(
        ("dict_large", {str(i): i for i in range(1000)})
    )
...
def marshal_equal(orig, reco):
    ...
    try:
        return type(orig) is type(reco) and orig == reco
...
class TestMarshalBoundaries(unittest.TestCase):
    ...
    def test_concatenated_stream_boundaries(self):
        ...
        for ver in range(self.max_version, 0, -1):
            ...
            with self.subTest(version=ver):
                self.assertEqual(
                    len(recovered), len(objects),
                    f"恢复对象数量 {len(recovered)} ≠ 期望 {len(objects)}"
                )

                for i, (orig, reco) in enumerate(zip(objects, recovered)):
                    self.assertTrue(
                        marshal_equal(orig, reco),
                        f"版本{ver}, 索引{i} ({names[i]}) 不匹配: "
                        f"{orig!r} ≠ {reco!r}"
                    )

                self.assertEqual(
                    boundaries[-1], len(blob),
                    "最后一个边界不等于拼接总长"
                )
    ...
    def test_deeply_nested_tuple(self):
        ...
        for ver in range(self.max_version, 0, -1):
            with self.subTest(
                version=ver, structure=f"deep_tuple_{DEPTH}"
            ):
                try:
                    data = marshal.dumps(obj, ver)
                except ValueError as e:
                    print(
                        f"  [版本 {ver}] 嵌套元组序列化被拒绝: {e}"
                    )
                    continue
    ...
    def test_large_object(self):
        ...
        for ver in range(self.max_version, 0, -1):
            with self.subTest(
                version=ver,
                size_mb=self.LARGE_OBJECT_SIZE // (1024 * 1024)
            ):
                start = time.time()
                try:
                ...
                self.assertLess(
                    elapsed, 10.0,
                    f"版本 {ver} 极大对象序列化过慢: {elapsed:.2f}s"
                )

    # ---------- 自动搜索递归深度边界 ----------
    def test_find_recursion_boundary(self):
        """
        对每个 marshal 格式版本，找出 == 比较不会触发 RecursionError
        的最大嵌套深度。同时提供详细的运行日志，便于诊断卡顿位置。
        """
        print("\n" + "=" * 60)
        ...
        for ver in range(self.max_version, 0, -1):
            ...
            for container_type in ("tuple",):
                ...
                if marshal_max == 0:
                    ...
                    with self.subTest(
                        version=ver, container=container_type
                    ):
